  ...
  return <SidebarProvider defaultOpen={!isMobile}>
      <div className="flex min-h-screen">
        <Sidebar variant="inset" className="border-r">
          <SidebarContent>
            {navigationItems.map(navGroup => <SidebarGroup key={navGroup.section}>
                <SidebarGroupLabel>{navGroup.section}</SidebarGroupLabel>
                <SidebarGroupContent>
                  <SidebarMenu>
                    {navGroup.items.map(item => <SidebarMenuItem key={item.title}>
        ...
        {isMobile && <div className="fixed top-4 left-4 z-10">
            <SidebarTrigger />
          </div>}
      </div>
    </SidebarProvider>;
